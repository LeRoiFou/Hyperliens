...
premiers caractères
La 6ème version prend en compte le téléchargement des données de la société française sur différents sites @ et la
version d'essai
...
import pygame  # pour la version d'essai
import datetime  # pour la version d'essai
...
class FrenchCompanies(Frame):
    ...
    def clipper(self):
        """Fonction permettant de copier le n° SIREN saisi"""

        window.clipboard_clear()
        window.clipboard_append(self.entrySiren.get())
    ...
    def widgets(self):
        ...
        """Bouton pour copier le n° SIREN"""
        self.btnSiren = Button(self, text='Copier n° SIREN', command=self.clipper)
        self.btnSiren.grid(row=13, column=2, pady=10)
...
"""Déclenchement de l'arrêt de la version d'essai"""
...
if now >= tictacboum:
    ...
    pygame.display.set_caption("Version d'essai finie !")  # titre de la fenêtre
    ...
    smiley_image = pygame.image.load('pieces/john.jpg')  # chargement du fichier
